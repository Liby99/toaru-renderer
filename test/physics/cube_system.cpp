    ...
    for (unsigned int i = 0; i < countX; i++) {
      for (unsigned int j = 0; j < countY; j++) {
        for (unsigned int k = 0; k < countZ; k++) {
          Vector3f pos =
            Vector3f(start(0, 0) + i * extent(0, 0) * 2.0,
                     start(1, 0) + j * extent(1, 0) * 2.0,
                     start(2, 0) + k * extent(2, 0) * 2.0);
          bool flipX, flipY, flipZ;
          flipX = (i % 2) == 0;
          flipY = (j % 2) == 0;
          flipZ = (k % 2) == 0;
          createUnitCube(pos, extent, 1.f, 380.0f, 0.1f, false, false, false); // flipX, flipY, flipZ);
        }
      }
    }
