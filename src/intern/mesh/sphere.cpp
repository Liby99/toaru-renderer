...
const int Sphere::SUBDIVISION = 5;

Sphere::Sphere() : Sphere(SUBDIVISION) {}

Sphere::Sphere(int subdivision) : subdivision(subdivision), Mesh() {}
...
void Sphere::generateMesh() {
  ...
  float s = sqrt(2.0f) / 4.0f;
  Vertices vertices;
  vertices.push_back(Vector3f(0, -0.5f, 0));
...
void Sphere::subdivide(Vertices &vertices, Triangles &triangles, int step) {
  ...
  for (int i = 0; i < numTriangles; i++) {
    ...
    vertices.push_back(m1.normalized() / 2.0f);
    vertices.push_back(m2.normalized() / 2.0f);
    vertices.push_back(m3.normalized() / 2.0f);
